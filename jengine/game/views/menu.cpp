...
Menu::Menu(const std::shared_ptr<SignalManager>& signal_receiver, const sf::Vector2f position, const sf::Vector2f size,
           const std::shared_ptr<sf::Texture> background_texture, const sf::Vector2f layout_position, const sf::Vector2f layout_size,
           const std::string& name )
  : graphics::View(signal_receiver, position, size, name)
{
  // Background
  ...
  // Main layout
  m_vlayout = std::make_shared<graphics::utils::VLayout>(signal_receiver, layout_position, layout_size, name + " - vlayout");
...
void Menu::setPressedEvent(std::shared_ptr<JDrawable> graphical_object, Callback callback)
{
  if(!graphical_object)
  {
